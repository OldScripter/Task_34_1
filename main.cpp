...
/**
 * @function Prints the vector with T type of elements.
 * @tparam T - type of elements
 * @param [in] vector - Vector for printing from.
 */
template <typename T>
...
int main()
{
    ...
    auto getFilesWithExtension = [](const std::filesystem::path& path, const std::string& extension)->std::unique_ptr<std::vector<std::filesystem::path>>
    {
        ...
        for(const auto& p: *paths)
        {
            ...
            {
                if (p.path().extension().compare(extension) == 0) // string::compare is 0 when compares are equal.
                {
